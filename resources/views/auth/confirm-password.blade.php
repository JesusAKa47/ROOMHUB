...
<head>
  ...
  <title>RoomHub | Confirmar contraseña</title>
...
<body class="min-h-screen bg-aurora text-white antialiased flex flex-col">
  ...
  <header class="container mx-auto px-6 py-5 flex items-center justify-between">
    <a href="{{ url('/') }}" class="flex items-center gap-3 group">
      <div class="h-9 w-9 rounded-xl bg-white/10 ring-soft grid place-items-center">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-indigo-300 group-hover:text-indigo-200 transition" fill="currentColor" viewBox="0 0 24 24">
  ...
  <main class="container mx-auto px-6 py-8 flex-1">
    <div class="mx-auto max-w-md glass ring-soft rounded-3xl p-6 sm:p-8">
      ...
      <h1 class="text-2xl font-semibold mb-1">Confirmar contraseña</h1>
      <p class="text-white/70 text-sm mb-6">
        Por seguridad necesitamos que ingreses tu contraseña antes de continuar.
      </p>
      ...
      <form method="POST" action="{{ route('password.confirm') }}" class="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="••••••••"
            class="w-full rounded-xl bg-white/10 border border-white/10 focus:border-indigo-400 focus:ring-2 
                   focus:ring-indigo-400/30 text-white placeholder-white/60 px-4 py-2 outline-none mt-1"
          >
